...
//        chatList.append("\(peerID.displayName): \(message)")
        ...
        receivedImage = UIImage(data: data)
...
extension ViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func sendVideoData(_ data: Data) {
        ...
        let numChunks = dataSize / chunkSize + 1
        for i in 0..<numChunks {
            let offset = i * chunkSize
            let chunk = data.subdata(in: offset..<min(offset + chunkSize, dataSize))
            do {
                try session.send(chunk, toPeers: peers, with: .unreliable)
            } catch {
                print("[Error] \(error)")
            }
        }
